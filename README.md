...
- **点开进单条连接的工作台**，左边时序梯形图（左客户端右服务端，每个包配一句人话：
  `客户端发起建连（SYN），起始编号 0`），右边 HTTP 报文，两栏各自独立滚动。
  点梯形图上的任意一个包，右边直接跳到并高亮它承载的那段报文
...
packages/
  ...
  web/      Vite + React，四个视图（上传 / 选 host / 连接列表 / 连接工作台），梯形图为手写 SVG
```
